...
HCAT_LOG_DIR=/var/log/hcat
HCAT_PID_DIR=/var/run/hcat
...
function print_usage() {
  ...
  echo "  status   Check status of HCatalog Server"
}

function status_hcat() {
  PID_FILE=${HCAT_PID_DIR}/hcat.pid
  if [ -f $PID_FILE ]; then
  	echo "hcat ( pid `cat $PID_FILE`) is running..."
  	exit 0
  else
  	echo "hcat is stopped"
  	exit 1
  fi
}
...
function start_hcat() {
  ...
  export HIVE_CONF_DIR=/etc/hive-metastore
...
#  HADOOP_OPTS="$HADOOP_OPTS -Dhive.metastore.warehouse.dir=$WAREHOUSE_DIR "
  ...
  export HADOOP_HEAPSIZE=256 # 8G is better if you have it
